...
private:
  ...
  std::vector<std::tuple<int, real_t, real_t>> FORCE_SIDES_;
  std::vector<std::tuple<int, real_t, real_t>> DIRI_SIDES_;
  bool SUPERVISED_LEARNING_;

public:
  /// @brief Constructor
  template <typename... Args>
  linear_elasticity(real_t lambda, real_t mu, bool SUPERVISED_LEARNING, int MAX_EPOCH,
                    real_t MIN_LOSS, std::vector<int> TFBC_SIDES,
                    std::vector<std::tuple<int, real_t, real_t>> FORCE_SIDES,
                    std::vector<std::tuple<int, real_t, real_t>> DIRI_SIDES,
  ...
  torch::Tensor loadDisplacements() {
      ...
      for (int i = 0; i < numCtrlPts; ++i) {
          matlabDisplacements[i][0] = matlabDisplacements_j[i][0].get<real_t>();
          matlabDisplacements[i][1] = matlabDisplacements_j[i][1].get<real_t>();
      }
  ...
  static std::vector<real_t> computeGrevilleAbscissae
  (const gismo::gsKnotVector<real_t>& knotVector, int degree, int numCtrlPts) {
      std::vector<real_t> greville(numCtrlPts, 0.0);
  ...
  static std::tuple<gismo::gsMatrix<real_t>, gismo::gsMatrix<real_t>, gismo::gsMatrix<real_t>> RunGismoSimulation(
        int64_t NR_CTRL_PTS, int DEGREE, real_t YOUNG_MODULUS, real_t POISSON_RATIO,
        const std::vector<std::tuple<int, real_t, real_t>>& DIRI_SIDES,
        const std::vector<std::tuple<int, real_t, real_t>>& FORCE_SIDES,
        const std::pair<real_t, real_t>& BODY_FORCE) {

    // initialize the reference control points and the calculated displacements & stresses
    gismo::gsMatrix<real_t> gsCtrlPts(NR_CTRL_PTS * NR_CTRL_PTS, 2);
    gismo::gsMatrix<real_t> gsDisplacements(NR_CTRL_PTS * NR_CTRL_PTS, 2);
    gismo::gsMatrix<real_t> gsStresses(gsCtrlPts.rows(), 1); //von Mises stresses (only one component)
    ...
    std::vector<real_t> grevilleU = computeGrevilleAbscissae(knotVector_u, DEGREE, NR_CTRL_PTS);
    std::vector<real_t> grevilleV = computeGrevilleAbscissae(knotVector_v, DEGREE, NR_CTRL_PTS);
    ...
    // create geometry
    gismo::gsTensorBSpline<2, real_t> geometry(knotVector_u, knotVector_v, gsCtrlPts);

    // create multipatch and add the geometry
    gismo::gsMultiPatch<real_t> multiPatch;
    multiPatch.addPatch(geometry);
    gismo::gsMultiBasis<real_t> basis(multiPatch);
    ...
    for (const auto& d : DIRI_SIDES) {
        ...
        real_t xVal = std::get<1>(d);
        real_t yVal = std::get<2>(d);
        auto gsSide = getGsBoundarySide(side);

        bcInfo.addCondition(0, gsSide, gismo::condition_type::dirichlet,
                            gismo::gsConstantFunction<real_t>(xVal, 2), 0);
        bcInfo.addCondition(0, gsSide, gismo::condition_type::dirichlet,
                            gismo::gsConstantFunction<real_t>(yVal, 2), 1);
    }

    // Neumann (Traction) BCs
    for (const auto& f : FORCE_SIDES) {
        int side = std::get<0>(f);
        real_t tx = std::get<1>(f);
        real_t ty = std::get<2>(f);
        auto gsSide = getGsBoundarySide(side);

        gismo::gsFunctionExpr<real_t> traction(std::to_string(tx), std::to_string(ty), 2);
        bcInfo.addCondition(0, gsSide, gismo::condition_type::neumann, traction);
    }

    // body force
    gismo::gsConstantFunction<real_t> bodyForce(BODY_FORCE.first, BODY_FORCE.second, 2);
    ...
    // solve the system
    typename gismo::gsSparseSolver<real_t>::CGDiagonal solver;
    ...
    for (int i = 0; i < gsCtrlPts.rows(); ++i) {
        // create temp point
        gismo::gsMatrix<real_t> point(2, 1);
  ...
  bool epoch(int64_t epoch) override {
    ...
    if (epoch == 0) {
      ...
      for (int i = 0; i < collPtsCoeffs.size(0); ++i) {
          collPtsCoeffs_j.push_back({collPtsCoeffs[i].item<real_t>()});
      }
  ...
  torch::Tensor loss(const torch::Tensor &outputs, int64_t epoch) override {
    ...
    if (SUPERVISED_LEARNING_ == false) {
        ...
        singleLossOutput << "EL " << std::setw(11) << elastLoss.item<real_t>();
        ...
        if (!TFBC_SIDES_.empty()) {
            ...
            totalLoss += *tfbcLoss;
            singleLossOutput << " + TL " << std::setw(11) << (*tfbcLoss).item<real_t>();
        }
        ...
        if (!FORCE_SIDES_.empty()) {
            ...
            totalLoss += *forceLoss;
            singleLossOutput << " + FL " << std::setw(11) << (*forceLoss).item<real_t>();
        }
        ...
        if (!DIRI_SIDES_.empty()) {
            ...
            totalLoss += *bcLoss;
            singleLossOutput << " + BL " << std::setw(11) << (*bcLoss).item<real_t>() / bcWeight
                             << " * 1e" << static_cast<int>(std::log10(bcWeight));
        }

        // print the loss values
        std::cout << std::setw(11) <<
            totalLoss.item<real_t>() << " = " << singleLossOutput.str() << std::endl;
    }
    ...
    // SUPERVISED LEARNING
        ...
        singleLossOutput << "GL " << std::setw(11) << (*gsLoss).item<real_t>()
    ...
    if ((epoch == MAX_EPOCH_ - 1) || (totalLoss.item<real_t>() <= MIN_LOSS_)) {
        ...
        int64_t size = jacobian[0]->size(0);
        ...
        torch::Tensor epsilon_xx = torch::zeros({size});
        torch::Tensor epsilon_yy = torch::zeros({size});
        torch::Tensor poisson_re = torch::zeros({size});
        ...
        for (int i = 0; i < size; ++i) {
            ...
            netVmStresses_j.push_back({sigma_vm[i].item<real_t>()});
            netXStresses_j.push_back({sigma_xx[i].item<real_t>()});
            netYStresses_j.push_back({sigma_yy[i].item<real_t>()});
            // add the poisson ratio to the json object
            netPoisson_j.push_back({poisson_re[i].item<real_t>()});
        }
        ...
        for (int i = 0; i < collPtsFirstAsTensor.size(0); ++i) {
            ...
            collPtsFirst_j.push_back({collPtsFirstAsTensor[i][0].item<real_t>(),
                                      collPtsFirstAsTensor[i][1].item<real_t>()});
            // new position of the collocation points
            collPtsFirstDispl_j.push_back({collPtsFirstAsTensor[i][0].item<real_t>() +
                                           displacementAsTensor[i][0].item<real_t>(),
                                           collPtsFirstAsTensor[i][1].item<real_t>() +
                                           displacementAsTensor[i][1].item<real_t>()});
        }
...
int main() {
  ...
  int NR_CTRL_PTS_REF = 8;
  int DEGREE_REF = 4;

  // spline parameters
  int64_t NR_CTRL_PTS = 8;  // in each direction
  constexpr int DEGREE = 4; // for geometry and variable
  ...
  std::pair<real_t, real_t> BODY_FORCE = {0.0, 0.0}; // {fx, fy}

  // --------------------------- //


  // calculation of lame parameters
  real_t lambda = (YOUNG_MODULUS * POISSON_RATIO) /
...
if (RUN_REF_SIM) {
    gismo::gsMatrix<real_t> gsRefCtrlPts;
    gismo::gsMatrix<real_t> gsRefDisplacements;
    gismo::gsMatrix<real_t> gsRefStresses;
    std::tie(gsRefCtrlPts, gsRefDisplacements, gsRefStresses) =
    linear_elasticity_t::RunGismoSimulation(NR_CTRL_PTS_REF, DEGREE,
        YOUNG_MODULUS, POISSON_RATIO, DIRI_SIDES, FORCE_SIDES, BODY_FORCE);
    gismo::gsMatrix<real_t> displacedGsRefCtrlPts = gsRefCtrlPts + gsRefDisplacements;
  ...
  for (int i = 0; i < NR_CTRL_PTS; ++i) {
      ctrlPtsCoeffs_j.push_back({ctrlPtsCoeffs[i].item<real_t>()});
  }
  ...
  for (const auto& side : DIRI_SIDES) {
      int sideNr = std::get<0>(side);
      real_t xDispl = std::get<1>(side);
      real_t yDispl = std::get<2>(side);
  ...
  // imposing body force
//   net.f().transform([=](const std::array<real_t, 2> xi) {
//       return std::array<real_t, 2>{0.0, 50.0};
//     });
//   change original geometry to wavy geometry
  ...
  gismo::gsMatrix<real_t> netCtrlPts(NR_CTRL_PTS * NR_CTRL_PTS, 2);
  // creating collection matrix for all the displacements (iganet)
  gismo::gsMatrix<real_t> netDisplacements(NR_CTRL_PTS * NR_CTRL_PTS, 2);
  ...
  for (int i = 0; i < NR_CTRL_PTS * NR_CTRL_PTS; ++i) {
      ...
      real_t y = geometryAsTensor[i + NR_CTRL_PTS * NR_CTRL_PTS].item<real_t>();
      netCtrlPts(i, 0) = x;
      netCtrlPts(i, 1) = y;

      real_t ux = displacementAsTensor[i].item<real_t>();
      real_t uy = displacementAsTensor[i + NR_CTRL_PTS * NR_CTRL_PTS].item<real_t>();
