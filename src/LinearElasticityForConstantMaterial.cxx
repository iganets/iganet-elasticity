...
class ElasticityForConstant : public iganet::IgANet2<Optimizer, Inputs, Outputs>,
    ...
    private:
        ...
        // constant material properties: E210 v0.3
    ...
    public:
        ...
        torch::Tensor loss(const torch::Tensor &outputs, int64_t epoch) override {
            ...
            if (!TFBC_SIDES_.empty() || !FORCE_SIDES_.empty()){   
                ...
                for (int side : neumannSides) {
                    ...
                    for (int i = 0; i < n_vals; ++i) {
                        ...
                        switch (side) {
                            case 1:
                                tractionValuesX[idx] =  - Lame1lambda * (ux_x[idx] + uy_y[idx]) - 2 * Lame2mu * ux_x[idx];    // \σ_11  == C11ij : epsiolnij = C_11idx : epsilon_idx      
                                tractionValuesY[idx] =  - Lame2mu * (uy_x[idx] + ux_y[idx]);                                     // \σ_12
                                break;
                            case 2:
                                tractionValuesX[idx] = Lame1lambda * (ux_x[idx] + uy_y[idx]) + 2 * Lame2mu * ux_x[idx];       // \σ_11
                                tractionValuesY[idx] = Lame2mu * (uy_x[idx] + ux_y[idx]);                                        // \σ_12
                                break;
                            case 3: 
                                tractionValuesX[idx] =  - Lame2mu * (uy_x[idx] + ux_y[idx]);                                     // \σ_21
                                tractionValuesY[idx] =  - Lame1lambda * (Jacobian_Nbdr(0,0)[idx] + uy_y[idx]) - 2 * Lame2mu * uy_y[idx];    // \σ_22
                                break;
                            case 4:
                                tractionValuesX[idx] = Lame2mu * (uy_x[idx] + ux_y[idx]);                                        // \σ_21
                                tractionValuesY[idx] = Lame1lambda * (ux_x[idx] + uy_y[idx]) + 2 * Lame2mu * uy_y[idx];       // \σ_22
                                break;
                            default:
            ...
            for (int i = 0; i < Hessian(0,0,0).size(0); ++i) {      // 36 it über interior 
                ...
                // x-direction
                divStressX[i] = (Lame1lambda + 2 * Lame2mu) * ux_xx[i] + 
                                Lame2mu * ux_yy[i] + (Lame1lambda + Lame2mu) * uy_xy[i];// +
                                // Ml_x[i] * (ux_x[i] + uy_y[i]) + 2 * Mm_x[i] * (ux_x[i]) + Mm_y[i] * (ux_y[i] + uy_x[i]);

                // y-direction
                divStressY[i] = Lame2mu * uy_xx[i] + (Lame1lambda + 2 * Lame2mu) * uy_yy[i] + 
                                (Lame1lambda + Lame2mu) * ux_xy[i] ;//+ 
            ...
            totalLoss = torch::tensor(0.0);
                ...
                // add  elasticity loss to  cmd-output variable EqLoss
            ...
            EqLoss_j.push_back( elastLoss.item<double>() );
            tfbcLoss_j.push_back( (*tfbcLoss).item<double>() );
        ...
        void PostProc() const{
            ...
            torch::Tensor epsilon_vm = torch::zeros({Jacobian[0]->size(0)});   
            ...
            nlohmann::json netVmStrain_j = nlohmann::json::array();
            ...
            for (int i = 0; i < Jacobian[0]->size(0); ++i) {        // 64 it über gesamte domain
                ...
                double a = ((epsilon_xx[i]-epsilon_yy[i])*(epsilon_xx[i]-epsilon_yy[i])).item<double>();
                double b = (epsilon_xx[i]*epsilon_xx[i]).item<double>();
                double c = (epsilon_yy[i]*epsilon_yy[i]).item<double>();
                double d = (epsilon_xy[i]*epsilon_xy[i]).item<double>();
                double e = (2.0/3.0) * ( a + b + c + 6*d );

                epsilon_vm[i] = sqrt( e );

                // calculate  stress values for all collocation points
                sigma_xx[i] = Lame1lambda * (epsilon_xx[i] + epsilon_yy[i]) + 2 * Lame2mu * epsilon_xx[i];
                sigma_xy[i] = Lame2mu * 2 * epsilon_xy[i];
                sigma_yy[i] = Lame1lambda * (epsilon_xx[i] + epsilon_yy[i]) + 2 * Lame2mu * epsilon_yy[i];
                ...
                netVmStrain_j.push_back({epsilon_vm[i].item<double>()});
            ...
            appendToJsonFile("net_VmStrain", netVmStrain_j);
            ...
            appendToJsonFile("EqLoss", EqLoss_j);
...
int main() {
    ...
    if (choice == "json") {
        ...
        std::cout << "NO CTRL PTS" << NR_CTRL_PTS << std::endl;
