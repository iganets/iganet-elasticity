...
def mapping2d(
    ...
    Indices i and j are 1-based, matching the original MATLAB implementation.
    NN/MM are structured with three rows per collocation index:
    basis value, first derivative, second derivative in parameter space.
    """
    ii = i - 1
    jj = j - 1
    ...
    ddN[0, :] = (
        ddN[0, :] / den_sum
        - (2 * dN[0, :] * der_sumx + N * der2_sumx) / den_sum**2
        + 2 * N * (der_sumx**2) / den_sum**3
    )
    ddN[1, :] = (
        ddN[1, :] / den_sum
        - (dN[0, :] * der_sumy + dN[1, :] * der_sumx + N * der2_sumxy) / den_sum**2
        + 2 * N * der_sumx * der_sumy / den_sum**3
    )
    ddN[2, :] = (
        ddN[2, :] / den_sum
        - (2 * dN[1, :] * der_sumy + N * der2_sumy) / den_sum**2
        + 2 * N * (der_sumy**2) / den_sum**3
    )
    ...
    d2xdxi2 = np.array(
        [
            [ddN[0, :] @ x, ddN[1, :] @ x, ddN[2, :] @ x],
            [ddN[0, :] @ y, ddN[1, :] @ y, ddN[2, :] @ y],
        ],
        dtype=float,
    )
    ...
    dxdxi2 = np.array(
        [
            ...
            [
                2 * dxdxi[0, 0] * dxdxi[1, 0],
                dxdxi[0, 0] * dxdxi[1, 1] + dxdxi[0, 1] * dxdxi[1, 0],
                2 * dxdxi[0, 1] * dxdxi[1, 1],
            ],
